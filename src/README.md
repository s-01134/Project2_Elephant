...
```
ofApp (Main Application)
├── SynthEngine (Audio Brain - Audio Thread)
│   ├── VoiceManager (Polyphonic Voice Pool)
│   │   ├── SineVoice x2
│   │   ├── SawVoice x2
│   │   ├── SquareVoice x2
│   │   ├── NoiseVoice x2
│   │   └── SamplerVoice x2 (shares kick.wav data)
│   ├── ThereminVoice (Continuous Pitch Control)
│   ├── EffectsChain
│   │   ├── LowPassFilter
│   │   └── DelayEffect
│   └── AudioRingBuffer (Thread-Safe Audio → Visualizer)
├── InputMapper (Keyboard → MIDI Notes)
├── InteractionPad (Mouse → Theremin + Gesture Recording)
└── Visualizer (Oscilloscope + Spectrum)
```

### Design Patterns

- **Composition**: ofApp owns all subsystems; VoiceManager owns voices
- **Inheritance**: All voice types inherit from `SynthVoice` abstract base
- **Association**: InputMapper and InteractionPad hold non-owning pointers to SynthEngine
- **Polymorphism**: Effects processed through abstract `Effect` interface
- **Thread Safety**: Atomic operations for cross-thread voice state access
...
### GUI Controls *(TODO: Requires KnobControl implementation)*
